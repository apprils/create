...
export function urlMapper(
  ...
  const path = (...args: PathChunk[]) => urlBuilder(options.base, setup.path, ...args)
  const href = (...args: PathChunk[]) => origin + path(...args)

  return {
    get name() { return setup.name },
...
export function urlBuilder(...args: PathChunk[]): string {
  return typeof args[args.length - 1] === "object"
    ? join(...args.slice(0, args.length - 1)) + "?" + stringify(args[args.length - 1] as {})
    : join(...args)
}
...
export function join(...args: PathChunk[]): string {
  return args.map(joinMapper).filter((e) => e).join("/").replace(/\/+/g, "/")
}
